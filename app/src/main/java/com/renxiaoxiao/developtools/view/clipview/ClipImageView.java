...
public class ClipImageView extends ImageView implements
  ...
  private final static int CROP_WIDTH = 300;
  private final static int CROP_HEIGHT = 300;
  ...
  public Bitmap clip() {
    ...
    draw(canvas);
//		Bitmap bmp = Bitmap.createBitmap(bitmap, mHorizontalPadding,
//				mVerticalPadding, getWidth() - 2 * mHorizontalPadding,
//				getWidth() - 2 * mHorizontalPadding);
//		return getCircleBitmap(bmp);
    Bitmap image =  Bitmap.createBitmap(bitmap, mHorizontalPadding,
      mVerticalPadding, (getWidth() - 2 * mHorizontalPadding),
            (getWidth() - 2 * mHorizontalPadding));
    Bitmap scaledBitmap = Bitmap.createScaledBitmap(image, CROP_WIDTH, CROP_HEIGHT, true);
    image.recycle();
    return scaledBitmap;
  }

  /**
   * 返回圆形图片，边角为透明的png图片格式
   * @param bitmap
   * @return
   */
